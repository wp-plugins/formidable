...
class FrmEntryMeta{
    ...
    function get_entry_metas_for_field($field_id, $order='', $limit='', $args=array()){
        ...
        if(!$is_draft)
            $query .= " INNER JOIN $frmdb->entries e ON (e.id=em.item_id) ";
    ...
    function getEntryIds($where = '', $order_by = '', $limit = '', $unique=true, $drafts=false){
        ...
        $query .= " FROM $frmdb->entry_metas it LEFT OUTER JOIN $frmdb->fields fi ON it.field_id=fi.id ";
    
        if(!$drafts){
            $query .= "INNER JOIN $frmdb->entries e ON (e.id=it.item_id) ";
            if(is_array($where))
                $where['e.is_draft'] = 0;
            else
