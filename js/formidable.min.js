...
function frmReplaceDefault(a,c){var e=a.replace(/(\n|\r\n)/g,"\r");""===jQuery(c).val()&&jQuery(c).addClass("frm_default").val(e)}
function frmCheckDependent(a,c,e){if("undefined"!=typeof __FRMRULES){var f=__FRMRULES[c];if("undefined"!=typeof f){if("undefined"==typeof e||null===e)e="go";for(var d=[],h=0;h<f.length;h++){var k=f[h];if("undefined"!=typeof k)for(var l=0;l<k.Conditions.length;l++){var m=k.Conditions[l];m.HideField=k.Setting.FieldName;m.MatchType=k.MatchType;m.Show=k.Show;d.push(m)}}for(var g=[],n=[],p=d.length,h=0;h<p;h++)(function(f){var b=d[f];"undefined"==typeof g[b.HideField]&&(g[b.HideField]=[]);if(b.FieldName!=
...
b.fkey+"_container:hidden").hide(),jQuery("#frm_field_"+b.fkey+"_container").hide()):jQuery("#frm_field_"+b.fkey+"_container").show():"show"==b.show?jQuery("#frm_field_"+b.fkey+"_container").show():(jQuery("#frm_field_"+b.fkey+"_container:hidden").hide(),jQuery("#frm_field_"+b.fkey+"_container").hide()),!1!==typeof b.result&&!0!==typeof b.result&&frmShowField(b.result,b.fname,e),delete n[a])})})(h)}}}
function frmOperators(a,c,e){"undefined"==typeof e&&(e="");jQuery.isArray(e)&&-1<jQuery.inArray(c,e)&&(e=c);-1!=String(c).search(/^\s*(\+|-)?((\d+(\.\d+)?)|(\.\d+))\s*$/)&&(c=parseFloat(c),e=parseFloat(e));return"-1"!=String(c).indexOf("&quot;")&&frmOperators(a,c.replace("&quot;",'"'),e)?!0:{"==":function(a,d){return a==d},"!=":function(a,d){return a!=d},"<":function(a,d){return a>d},">":function(a,d){return a<d},LIKE:function(a,d){return-1!=d.indexOf(a)},"not LIKE":function(a,d){return-1==d.indexOf(a)}}[a](c,
...
jQuery(a).find('input[name="form_id"]').val()+"_container");e=c.offset().top;c.replaceWith(d);f=document.documentElement.scrollTop||document.body.scrollTop;e&&0<e&&f>e&&jQuery(window).scrollTop(e);if("function"==typeof frmThemeOverride_frmAfterSubmit){var h=jQuery(d).find('input[name="form_id"]').val(),k="";h&&(k=jQuery('input[name="frm_page_order_'+h+'"]').val());frmThemeOverride_frmAfterSubmit(h,k,d)}jQuery(a).find('input[name="id"]').length&&(d=jQuery(a).find('input[name="id"]').val(),jQuery("#frm_edit_"+
d).find("a").addClass("frm_ajax_edited").click())}else{jQuery(a).find('input[type="submit"], input[type="button"]').removeAttr("disabled");jQuery(a).find(".frm_ajax_loading").css("visibility","hidden");h=!0;jQuery(".form-field").removeClass("frm_blank_field");jQuery(".form-field .frm_error").replaceWith("");c="";var k=!1,l;for(l in d)if(jQuery(a).find("#frm_field_"+l+"_container").length&&jQuery("#frm_field_"+l+"_container").is(":visible")){h=!1;if(""===c){c="#frm_field_"+l+"_container";e=jQuery(a).find(c).offset().top;
var m=jQuery("html").css("margin-top");e&&m&&(e-=parseInt(m));f=document.documentElement.scrollTop||document.body.scrollTop;e&&f>e&&jQuery(window).scrollTop(e-4)}jQuery(a).find("#frm_field_"+l+"_container #recaptcha_area").length&&(k=!0,Recaptcha.reload());jQuery(a).find("#frm_field_"+l+"_container").addClass("frm_blank_field");"function"==typeof frmThemeOverride_frmPlaceError?frmThemeOverride_frmPlaceError(l,d):jQuery(a).find("#frm_field_"+l+"_container").append('<div class="frm_error">'+d[l]+"</div>")}else if("redirect"==
l){window.location=d[l];return}!0!==k&&jQuery(a).find("#recaptcha_area").replaceWith("");h&&a.submit()}},error:function(){jQuery(a).find('input[type="submit"], input[type="button"]').removeAttr("disabled");a.submit()}})}
function frmEditEntry(a,c,e,f,d,h){var k=jQuery("#frm_edit_"+a).text(),l=jQuery("#"+c+a).html();jQuery("#"+c+a).html('<span class="frm-loading-img" id="'+c+a+'"></span><div class="frm_orig_content" style="display:none">'+l+"</div>");jQuery.ajax({type:"POST",url:frm_js.ajax_url,dataType:"html",data:"action=frm_entries_edit_entry_ajax&post_id="+e+"&entry_id="+a+"&id="+f,success:function(l){jQuery("#"+c+a).children(".frm-loading-img").replaceWith(l);jQuery("#frm_edit_"+a).replaceWith('<span id="frm_edit_'+
a+'"><a onclick="frmCancelEdit('+a+",'"+c+"','"+k+"',"+e+","+f+",'"+h+'\')" class="'+h+'">'+d+"</a></span>")}})}
function frmCancelEdit(a,c,e,f,d,h){var k=jQuery("#frm_edit_"+a).text();jQuery("#frm_edit_"+a).find("a").hasClass("frm_ajax_edited")||(jQuery("#"+c+a).children(".frm_forms").replaceWith(""),jQuery("#"+c+a).children(".frm_orig_content").fadeIn("slow").removeClass("frm_orig_content"));jQuery("#frm_edit_"+a).replaceWith('<a id="frm_edit_'+a+'" class="frm_edit_link '+h+'" href="javascript:frmEditEntry('+a+",'"+c+"',"+f+","+d+",'"+k+"','"+h+"')\">"+e+"</a>")}
...
function frm_resend_email(a,c){jQuery("#frm_resend_email").replaceWith('<img id="frm_resend_email" src="'+frm_js.images_url+'/wpspin_light.gif" alt="'+frm_js.loading+'" />');jQuery.ajax({type:"POST",url:frm_js.ajax_url,data:"action=frm_entries_send_email&entry_id="+a+"&form_id="+c+"&type=email",success:function(a){jQuery("#frm_resend_email").replaceWith(a)}})}function frmScrollMsg(a){(a=jQuery("#frm_form_"+a+"_container").offset())&&window.scrollTo(a.left,a.top-28)}
jQuery.fn.frmVisible=function(){return this.css("visibility","visible")};jQuery.fn.frmInvisible=function(){return this.css("visibility","hidden")};jQuery.fn.frmVisibilityToggle=function(){return this.css("visibility",function(a,c){return"visible"==c?"hidden":"visible"})};