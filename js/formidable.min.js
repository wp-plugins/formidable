jQuery(document).ready(function(a){a(".frm_ajax_loading").css("visibility","hidden");var c=a(".frm_blank_field").closest(".frm_toggle_container").prev(".frm_trigger");c&&frmToggleSection(c);a.isFunction(a.fn.placeholder)?a(".frm-show-form input, .frm-show-form textarea").placeholder():jQuery(".frm-show-form input[onblur], .frm-show-form textarea[onblur]").each(function(){""==jQuery(this).val()&&jQuery(this).blur()});a(document).on("click",'.frm-show-form input[type="submit"], .frm-show-form input[name="frm_prev_page"], .frm-show-form .frm_save_draft',
frmSetNextPage);a(document).on("click",".frm_remove_link",frmRemoveDiv)});
...
(c=jQuery(a).find("input[type=file]").val(),"undefined"!=typeof c&&""!=c&&window.setTimeout(function(){jQuery("#frm_loading").hide().fadeIn()},2E3)),jQuery(a).find("#recaptcha_area").length&&(1>jQuery(a).find(".frm_next_page").length||1>jQuery(a).find(".frm_next_page").val())&&jQuery(a).find("#recaptcha_area").replaceWith(""),a.submit();else if("object"!=typeof c){jQuery(a).find(".frm_ajax_loading").css("visibility","hidden");var d=jQuery(a).closest("#frm_form_"+jQuery(a).find('input[name="form_id"]').val()+
"_container"),e=d.offset().top;d.replaceWith(c);var f=document.documentElement.scrollTop||document.body.scrollTop;e&&0<e&&f>e&&jQuery(window).scrollTop(e);if("function"==typeof frmThemeOverride_frmAfterSubmit){if(d=jQuery(c).find('input[name="form_id"]').val())var h=jQuery('input[name="frm_page_order_'+d+'"]').val();frmThemeOverride_frmAfterSubmit(d,h,c)}jQuery(a).find('input[name="id"]').length&&(c=jQuery(a).find('input[name="id"]').val(),jQuery("#frm_edit_"+c).find("a").addClass("frm_ajax_edited").click())}else{jQuery(a).find('input[type="submit"], input[type="button"]').removeAttr("disabled");
