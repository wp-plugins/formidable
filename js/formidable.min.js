...
0==e.length&&(e="");jQuery.ajax({type:"POST",url:b,data:"controller=fields&frm_action=ajax_data_options&hide_field="+h+"&entry_id="+d+"&selected_field_id="+a.LinkedField+"&field_id="+a.HideField,success:function(b){""==b?jQuery("#frm_field_"+a.HideField+"_container").hide():"all"!=a.MatchType&&jQuery("#frm_field_"+a.HideField+"_container").show();jQuery("#frm_data_field_"+a.HideField+"_container").html(b);jQuery(b).hasClass("frm_chzn")&&jQuery(".frm_chzn").chosen();""!=b&&""!=e&&jQuery.each(e,function(b,
d){"checkbox"==a.DataType?jQuery("#field_"+a.HideField+"-"+d).attr("checked","checked"):"select"==a.DataType?jQuery("select[name='item_meta["+a.HideField+"]"+f+"']").val(d):jQuery("input[name='item_meta["+a.HideField+"]']").val(d)});frmCheckDependent(e,a.HideField)}})}function frmOnSubmit(a){a.preventDefault();jQuery(this).find(".wp-editor-wrap").length&&tinyMCE.triggerSave();frmGetFormErrors(this)}
