...
function frmCheckDependent(a,d){if("undefined"!=typeof __FRMRULES)var b=__FRMRULES;if("undefined"!=typeof __FRMURL)var h=__FRMURL;if("undefined"!=typeof b&&(b=b[d],"undefined"!=typeof b)){for(var e=[],f=0;f<b.length;f++){var j=b[f];if("undefined"!=typeof j)for(var l=0;l<j.Conditions.length;l++){var k=j.Conditions[l];k.HideField=j.Setting.FieldName;k.MatchType=j.MatchType;k.Show=j.Show;e.push(k)}}for(var g=[],m=[],n=e.length,f=0;f<n;f++)(function(b){var c=e[b];"undefined"==typeof g[c.HideField]&&(g[c.HideField]=
...
!1,jQuery("#frm_field_"+c.HideField+"_container").fadeOut("slow"),jQuery("#frm_data_field_"+c.HideField+"_container").html("")):g[c.HideField][b]="undefined"==typeof c.DataType?frmOperators(c.Condition,c.Value,a):{funcName:"frmGetData",f:c,sel:a}:g[c.HideField][b]=""==a?!1:{funcName:"frmGetDataOpts",f:c,sel:a};else if("data-checkbox"==c.Type){var j=[];jQuery("input[name='item_meta["+c.FieldName+"][]']:checked, input[type='hidden'][name='item_meta["+c.FieldName+"][]']").each(function(){j.push(jQuery(this).val())});
"undefined"==typeof c.DataType||""==c.DataType||"data"==c.DataType?0==j.length?(g[c.HideField][b]=!1,jQuery("#frm_field_"+c.HideField+"_container").fadeOut("slow"),jQuery("#frm_data_field_"+c.HideField+"_container").html("")):(g[c.HideField][b]=!0,jQuery("#frm_data_field_"+c.HideField+"_container").html(""),frmGetData(c,j,h,1)):g[c.HideField][b]=0==j.length?!1:{funcName:"frmGetDataOpts",f:c,sel:j}}else"data-select"==c.Type&&"undefined"!=typeof c.LinkedField?""==c.DataType||"data"==c.DataType?""==
...
!0!=typeof b.result&&frmShowField(b.result,h,b.fname),delete m[a])})})(f)}}
...
function frmGetData(a,d,b,h){h||jQuery("#frm_field_"+a.HideField+"_container").html('<span class="frm-loading-img"></span>');jQuery.ajax({type:"POST",url:b,data:"controller=fields&frm_action=ajax_get_data&entry_id="+d+"&field_id="+a.LinkedField+"&current_field="+a.HideField,success:function(b){""!=b&&jQuery("#frm_field_"+a.HideField+"_container").show();if(h)jQuery("#frm_data_field_"+a.HideField+"_container").append(b);else{jQuery("#frm_data_field_"+a.HideField+"_container").html(b);var f=jQuery("#frm_data_field_"+
...
function frmGetDataOpts(a,d,b,h){jQuery("#frm_data_field_"+a.HideField+"_container").html('<span class="frm-loading-img"></span>');var e=[];if("checkbox"==a.DataType||"radio"==a.DataType)jQuery("input[name='item_meta["+a.HideField+"][]']:checked").each(function(){e.push(jQuery(this).val())});else if("select"==a.DataType){var f=0<jQuery("select[name='item_meta["+a.HideField+"][]']").length?"[]":"";e.push(jQuery("select[name='item_meta["+a.HideField+"]"+f+"']").val())}else e.push(jQuery("input[name='item_meta["+
a.HideField+"]']").val());0==e.length&&(e="");jQuery.ajax({type:"POST",url:b,data:"controller=fields&frm_action=ajax_data_options&hide_field="+h+"&entry_id="+d+"&selected_field_id="+a.LinkedField+"&field_id="+a.HideField,success:function(b){""==b?jQuery("#frm_field_"+a.HideField+"_container").hide():"all"!=a.MatchType&&jQuery("#frm_field_"+a.HideField+"_container").show();jQuery("#frm_data_field_"+a.HideField+"_container").html(b);jQuery(b).hasClass("frm_chzn")&&jQuery(".frm_chzn").chosen();""!=b&&
""!=e&&jQuery.each(e,function(b,d){"checkbox"==a.DataType?jQuery("#field_"+a.HideField+"-"+d).attr("checked","checked"):"select"==a.DataType?jQuery("select[name='item_meta["+a.HideField+"]"+f+"']").val(d):jQuery("input[name='item_meta["+a.HideField+"]']").val(d)});frmCheckDependent("",a.HideField)}})}
...
var f=document.documentElement.scrollTop||document.body.scrollTop;e&&(0<e&&f>e)&&jQuery(window).scrollTop(e);if("function"==typeof frmThemeOverride_frmAfterSubmit){if(d=jQuery(b).find('input[name="form_id"]').val())var j=jQuery('input[name="frm_page_order_'+d+'"]').val();frmThemeOverride_frmAfterSubmit(d,j,b)}0<jQuery(a).find('input[name="id"]').length&&(b=jQuery(a).find('input[name="id"]').val(),jQuery("#frm_edit_"+b).find("a").addClass("frm_ajax_editted").click())}else{jQuery(a).find('input[type="submit"]').removeAttr("disabled");
jQuery(a).find(".frm_ajax_loading").css("visibility","hidden");j=!0;jQuery(".form-field").removeClass("frm_blank_field");jQuery(".form-field .frm_error").replaceWith("");var d="",l=!1,k;for(k in b)jQuery(a).find("#frm_field_"+k+"_container").length&&jQuery("#frm_field_"+k+"_container").is(":visible")&&(j=!1,""==d&&(d="#frm_field_"+k+"_container",e=jQuery(a).find(d).offset().top,f=jQuery("html").css("margin-top"),e&&f&&(e-=parseInt(f)),f=document.documentElement.scrollTop||document.body.scrollTop,
e&&f>e&&jQuery(window).scrollTop(e-4)),jQuery(a).find("#frm_field_"+k+"_container #recaptcha_area").length&&(l=!0,Recaptcha.reload()),jQuery(a).find("#frm_field_"+k+"_container").addClass("frm_blank_field"),"function"==typeof frmThemeOverride_frmPlaceError?frmThemeOverride_frmPlaceError(k,b):jQuery(a).find("#frm_field_"+k+"_container").append('<div class="frm_error">'+b[k]+"</div>"));!0!=l&&jQuery(a).find("#recaptcha_area").replaceWith("");j&&a.submit()}},error:function(){jQuery(a).find('input[type="submit"]').removeAttr("disabled");
...
function frmEditEntry(a,d,b,h,e,f,j){var l=jQuery("#frm_edit_"+a).text(),k=jQuery("#"+b+a).html();jQuery("#"+b+a).html('<span class="frm-loading-img" id="'+b+a+'"></span><div class="frm_orig_content" style="display:none">'+k+"</div>");jQuery.ajax({type:"POST",url:d,dataType:"html",data:"controller=entries&frm_action=edit_entry_ajax&post_id="+h+"&entry_id="+a+"&id="+e,success:function(g){jQuery("#"+b+a).children(".frm-loading-img").replaceWith(g);jQuery("#frm_edit_"+a).replaceWith('<span id="frm_edit_'+
a+'"><a onclick="frmCancelEdit('+a+",'"+b+"','"+l+"','"+d+"',"+h+","+e+",'"+j+'\')" class="'+j+'">'+f+"</a></span>")}})}
function frmCancelEdit(a,d,b,h,e,f,j){var l=jQuery("#frm_edit_"+a).text();jQuery("#frm_edit_"+a).find("a").hasClass("frm_ajax_editted")||(jQuery("#"+d+a).children(".frm_forms").replaceWith(""),jQuery("#"+d+a).children(".frm_orig_content").fadeIn("slow").removeClass("frm_orig_content"));jQuery("#frm_edit_"+a).replaceWith('<a id="frm_edit_'+a+'" class="frm_edit_link '+j+'" href="javascript:frmEditEntry('+a+",'"+h+"','"+d+"',"+e+","+f+",'"+l+"','"+j+"')\">"+b+"</a>")}
function frmUpdateField(a,d,b,h,e,f){jQuery("#frm_update_field_"+a+"_"+d).html('<span class="frm-loading-img"></span>');jQuery.ajax({type:"POST",url:e,data:"controller=entries&frm_action=update_field_ajax&entry_id="+a+"&field_id="+d+"&value="+b,success:function(){""==h?jQuery("#frm_update_field_"+a+"_"+d+"_"+f).fadeOut("slow"):jQuery("#frm_update_field_"+a+"_"+d+"_"+f).replaceWith(h)}})}
