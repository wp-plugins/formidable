...
a.LinkedField+"&field_id="+a.HideField,success:function(c){""==c?(jQuery("#frm_field_"+a.HideField+"_container").hide(),f=""):"all"!=a.MatchType&&jQuery("#frm_field_"+a.HideField+"_container").show();jQuery("#frm_data_field_"+a.HideField+"_container").html(c);if(""!=c&&""!=f){if(!jQuery.isArray(f)){var d=[];d.push(f);f=d}jQuery.each(f,function(c,d){"undefined"!=typeof d&&("checkbox"==a.DataType||"radio"==a.DataType?jQuery("#field_"+a.HideField+"-"+d).attr("checked","checked"):"select"==a.DataType?
jQuery("select[name^='item_meta["+a.HideField+"]'] option[value="+d+"]").length?jQuery("select[name^='item_meta["+a.HideField+"]'] option[value="+d+"]").prop("selected",!0):f.splice(c,1):jQuery("input[name^='item_meta["+a.HideField+"]']").val(d))})}jQuery(c).hasClass("frm_chzn")&&jQuery().chosen&&jQuery(".frm_chzn").chosen();frmCheckDependent(f,a.HideField,"stop")}})}}
