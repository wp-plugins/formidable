jQuery(document).ready(function(a){a(".frm_ajax_loading").css("visibility","hidden");var c=a(".frm_blank_field").closest(".frm_toggle_container").prev(".frm_trigger");c&&frmToggleSection(c);a.isFunction(a.fn.placeholder)?a(".frm-show-form input, .frm-show-form textarea").placeholder():jQuery(".frm-show-form input[onblur], .frm-show-form textarea[onblur]").each(function(){""==jQuery(this).val()&&jQuery(this).blur()});if(a.isFunction(a.fn.on))a(document).on("click",'.frm-show-form input[type="submit"], .frm-show-form input[name="frm_prev_page"], .frm-show-form .frm_save_draft',
frmSetNextPage);else a('.frm-show-form input[type="submit"], .frm-show-form input[name="frm_prev_page"], .frm-show-form .frm_save_draft').live("click",frmSetNextPage)});
function frmSetNextPage(a){a.preventDefault();a=jQuery(this).parents("form:first");if("frm_prev_page"==jQuery(this).attr("name")||jQuery(this).hasClass("frm_prev_page"))var c=jQuery(a).find(".frm_next_page").attr("id").replace("frm_next_p_",""),d="";else"frm_save_draft"==jQuery(this).attr("name")||jQuery(this).hasClass("frm_save_draft")?(c="",d=1):d=c="";jQuery(".frm_next_page").val(c);jQuery(".frm_saving_draft").val(d);a.trigger("submit")}
...
jQuery.fn.frmVisibilityToggle=function(){return this.css("visibility",function(a,c){return"visible"==c?"hidden":"visible"})};function frmScrollFrm(a){(a=jQuery("#frm_form_"+a+"_container").offset())&&window.scrollTo(a.left,a.top-28)};