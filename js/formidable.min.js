jQuery(document).ready(function(a){a(".frm_ajax_loading").css("visibility","hidden");var c=a(".frm_blank_field").closest(".frm_toggle_container").prev(".frm_trigger");c&&frmToggleSection(c);if(a.isFunction(a.fn.on))a(document).on("click",'.frm-show-form input[type="submit"]',function(){if("frm_prev_page"==a(this).attr("name")){var c=a(this).parents("form:first"),c=a(c).find(".frm_next_page").attr("id").replace("frm_next_p_","");a(".frm_next_page").val(c)}else a(".frm_next_page").val("")});else a('.frm-show-form input[type="submit"]').live("click",
function(){if("frm_prev_page"==a(this).attr("name")){var c=a(this).parents("form:first"),c=a(c).find(".frm_next_page").attr("id").replace("frm_next_p_","");a(".frm_next_page").val(c)}else a(".frm_next_page").val("")})});
...
function frmGetFormErrors(a){jQuery(a).find('input[type="submit"]').attr("disabled","disabled");jQuery(a).find(".frm_ajax_loading").css("visibility","visible");jQuery.ajax({type:"POST",url:frm_js.ajax_url,data:jQuery(a).serialize()+"&action=frm_entries_"+jQuery(a).find('input[name="frm_action"]').val()+"&_ajax_nonce=1",success:function(c){c=c.replace(/^\s+|\s+$/g,"");0===c.indexOf("{")&&(c=jQuery.parseJSON(c));if(""==c||!c||"0"==c||"object"!=typeof c&&0===c.indexOf("<!DOCTYPE"))jQuery("#frm_loading").length&&
...
"_container"),e=d.offset().top;d.replaceWith(c);var f=document.documentElement.scrollTop||document.body.scrollTop;e&&0<e&&f>e&&jQuery(window).scrollTop(e);if("function"==typeof frmThemeOverride_frmAfterSubmit){if(d=jQuery(c).find('input[name="form_id"]').val())var h=jQuery('input[name="frm_page_order_'+d+'"]').val();frmThemeOverride_frmAfterSubmit(d,h,c)}0<jQuery(a).find('input[name="id"]').length&&(c=jQuery(a).find('input[name="id"]').val(),jQuery("#frm_edit_"+c).find("a").addClass("frm_ajax_edited").click())}else{jQuery(a).find('input[type="submit"]').removeAttr("disabled");
