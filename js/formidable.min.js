...
function frmUpdateField(a,d,b,h,e,f){jQuery("#frm_update_field_"+a+"_"+d).html('<span class="frm-loading-img"></span>');jQuery.ajax({type:"POST",url:e,data:"controller=entries&frm_action=update_field_ajax&entry_id="+a+"&field_id="+d+"&value="+b,success:function(b){""==h.replace(/^\s+|\s+$/g,"")?jQuery("#frm_update_field_"+a+"_"+d+"_"+f).fadeOut("slow"):jQuery("#frm_update_field_"+a+"_"+d+"_"+f).replaceWith(h)}})}
function frmDeleteEntry(a,d,b){jQuery("#frm_delete_"+a).replaceWith('<span class="frm-loading-img" id="frm_delete_'+a+'"></span>');jQuery.ajax({type:"POST",url:d,data:"controller=entries&frm_action=destroy&entry="+a,success:function(d){"success"==d.replace(/^\s+|\s+$/g,"")?jQuery("#"+b+a).fadeOut("slow"):jQuery("#frm_delete_"+a).replaceWith(d)}})}function frmRemoveDiv(a){jQuery(a).fadeOut("slow").replaceWith("")}
