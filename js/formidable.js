...
function frm_check_dependent(selected,type,field_id,opts,ajax_url){
var atts=String(opts).split('||');
if(atts.length==1){var this_opts=new Array();this_opts[0]=opts;}
else{var this_opts=atts;}
for(i=0; i<this_opts.length; i++){
    var field_data = String(this_opts[i]).split('|');
    if(type=='checkbox'){
        var show_field=false;
        jQuery("input[name='item_meta["+field_id+"][]']:checked").each(function(){if(show_field==false && jQuery(this).val() == field_data[1]){show_field=true;jQuery('#frm_field_'+field_data[0]+'_container').fadeIn('slow');}});
        ...
        else{jQuery.ajax({type:"POST",url:ajax_url,data:"controller=fields&action=ajax_get_data&entry_id="+entry_id+"&field_id="+field_data[1],success:function(html){jQuery('#frm_field_'+field_data[0]+'_container').html(html);}});
        }
    }else if(type=='data-checkbox'){
        var replace_it=false;
        jQuery("input[name='item_meta["+field_id+"][]']:checked").each(function(){
        if(selected!=''){jQuery.ajax({type:"POST",url:ajax_url,data:"controller=fields&action=ajax_get_data&entry_id="+selected+"&field_id="+field_data[1],success:function(html){jQuery('#frm_data_field_'+field_data[0]+'_container').append(html);replace_it=true;}});
        }
        });
        ...
        if(field_data[2]=='' || field_data[2]=='data'){
            ...
            else{jQuery.ajax({type:"POST",url:ajax_url,data:"controller=fields&action=ajax_get_data&entry_id="+selected+"&field_id="+field_data[1],success:function(html){jQuery('#frm_data_field_'+field_data[0]+'_container').html(html);}});}
        }else{
            if(selected==''){jQuery('#frm_field_'+field_data[0]+'_container').fadeOut('slow');}
            else{jQuery.ajax({type:"POST",url:ajax_url,data:"controller=fields&action=ajax_data_options&hide_field="+field_id+"&entry_id="+selected+"&selected_field_id="+field_data[1]+"&field_id="+field_data[0],success:function(html){jQuery('#frm_field_'+field_data[0]+'_container').fadeIn('slow'); jQuery('#frm_data_field_'+field_data[0]+'_container').html(html);}});};
        }
    }else{
